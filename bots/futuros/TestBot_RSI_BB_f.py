...
class FuturesTrader():
    ...
    def __init__(self, symbol, bar_length, rsi_length, rsi_overbought,rsi_oversold, sma, dev, units, position = 0, leverage = 5):
...
if __name__ == "__main__": # only if we run trader.py as a script, please do the following:
    ...
    trader = FuturesTrader(symbol = symbol, bar_length = bar_length, rsi_length = rsi_length,
                       rsi_overbought = rsi_overbought, rsi_oversold = rsi_oversold, 
                       sma = sma, dev = dev, units = units, position = position)
