...
class FuturesTrader():
    ...
    def __init__(self, symbol, bar_length, periods, Stoch_overbought,Stoch_oversold, moving_av, sma, units, position = 0):
    ...
    #******************** define your strategy here ************************
        ...
        data['bollinger_first'] = data.Close.rolling(window = self.SMA).mean() + 2*(data.Close.rolling(self.SMA).std())
        data['bollinger_second'] = data.Close.rolling(window = self.SMA).mean() - 2*(data.Close.rolling(self.SMA).std())
...
if __name__ == "__main__": # only if we run trader.py as a script, please do the following:
    ...
    trader = FuturesTrader(symbol = symbol, bar_length = bar_length, periods = periods,
