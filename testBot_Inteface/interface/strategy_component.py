...
from strategies import MACD_RSI, BB_RSI, BB_OStochastic, BreakoutStrategy
from utils import *
...
class StrategyEditor(tk.Frame):
    def __init__(self, root: "Root", binance: BinanceClient, bitmex: BitmexClient, *args, **kwargs):
        ...
        self._base_params = [
            {"code_name": "strategy_type", "widget": tk.OptionMenu, "data_type": str,
             "values": ["Technical1","Technical2","Technical3", "Breakout"], "width": 12, "header": "Strategy"},
            ...
            {"code_name": "take_profit", "widget": tk.Entry, "data_type": float, "width": 6, "header": "TP %"},
            {"code_name": "stop_loss", "widget": tk.Entry, "data_type": float, "width": 6, "header": "SL %"},
        ...
        self.extra_params = {
            "Technical1": [
             ...
             "Technical2": [
                {"code_name": "rsi_length", "name": "RSI Periods", "widget": tk.Entry, "data_type": int},
                {"code_name": "sma", "name": "BBollinger Length", "widget": tk.Entry, "data_type": int},
                {"code_name": "mult_BB", "name": "Multiplo", "widget": tk.Entry, "data_type": int},


            ],
            "Technical3": [
                {"code_name": "period", "name": "Stoch Periods", "widget": tk.Entry, "data_type": int},
                {"code_name": "d_period", "name": "%D periods", "widget": tk.Entry, "data_type": int},
                {"code_name": "sma", "name": "BBollinger Length", "widget": tk.Entry, "data_type": int},
                {"code_name": "mult_BB", "name": "Multiplo", "widget": tk.Entry, "data_type": int},
    ...
    def _switch_strategy(self, b_index: int):
        ...
        if self.body_widgets['activation'][b_index].cget("text") == "OFF":
            ...
            if strat_selected == "Technical1":
                new_strategy = MACD_RSI(self._exchanges[exchange], contract, exchange, timeframe, balance_pct,
                                                 take_profit, stop_loss, self.additional_parameters[b_index])
            elif strat_selected == "Technical2":
                new_strategy = BB_RSI(self._exchanges[exchange], contract, exchange, timeframe, balance_pct,
                                                 take_profit, stop_loss, self.additional_parameters[b_index])
            elif strat_selected == "Technical3":
                new_strategy = BB_OStochastic(self._exchanges[exchange], contract, exchange, timeframe, balance_pct,
                                                 take_profit, stop_loss, self.additional_parameters[b_index])
