...
class MACD_RSI(Strategy):
    def __init__(self, client, contract: Contract, exchange: str, timeframe: str, balance_pct: float, take_profit: float,
        ...
        super().__init__(client, contract, exchange, timeframe, balance_pct, take_profit, stop_loss, "Technical1")
...
class BB_RSI(Strategy):
    def __init__(self, client, contract: Contract, exchange: str, timeframe: str, balance_pct: float, take_profit: float,
                 stop_loss: float, other_params: Dict):
        super().__init__(client, contract, exchange, timeframe, balance_pct, take_profit, stop_loss, "Technical2")

        self._sma =  other_params['sma']
        self._mult_BB = other_params ['mult_BB']
        self._rsi_length = other_params['rsi_length']
    ...
    def _bb(self) -> Tuple[float, float]:

        """
        Compute the Bollinger Bands.
        :return: The bollinger_first and the bollinger_firs value of the previous candlestick
        """
        ...
        sma = closes.rolling(window = self._sma).mean() 
        bollinger_first = sma + (self._mult_BB)*(closes.rolling(self._sma).std())
        bollinger_second = sma - (self._mult_BB)*(closes.rolling(self._sma).std())

        return bollinger_first.iloc[-2], bollinger_second.iloc[-2]
    ...
    def _check_signal(self):
        ...
        bollinger_first, bollinger_second = self._bb()
        rsi = self._rsi()

        if rsi < 30 and self.candles[-1].close  > bollinger_second :
            return 1
        elif rsi > 70 and self.candles[-1].close  <  bollinger_first:
            return -1
        else:
            return 0
...
class BB_OStochastic(Strategy):
    def __init__(self, client, contract: Contract, exchange: str, timeframe: str, balance_pct: float, take_profit: float,
                 stop_loss: float, other_params: Dict):
        super().__init__(client, contract, exchange, timeframe, balance_pct, take_profit, stop_loss, "Technical3")

        self._sma =  other_params['sma']
        self._mult_BB = other_params ['mult_BB']
        self._period =  other_params ['period']
        self._d_period = other_params ['d_period']

    def _Stoch(self) -> Tuple[float, float]:

        """
        Compute the O. Stochastic.
        :return: The  %K and %D value of the previous candlestick
        """
        close_list = []
        for candle in self.candles:
            close_list.append(candle.close) 
        ...
        roll_low = lows.rolling(self._period).min()
        roll_high = highs.rolling(self._period).max()
        K_fast = (closes - roll_low) / (roll_high - roll_low) * 100 #Fast Stochastic Indicator(%K Line) 
        D_slow = K_fast.rolling(self._d_period).mean() #Slow Stochastic Indicator(%D Line)

        return K_fast.iloc[-2], D_slow.iloc[-2]
    ...
    def _check_signal(self):
        ...
        :return: 1 for a Long signal, -1 for a Short signal, 0 for no signal
        """
        K_fast, D_slow = self._Stoch()
        bollinger_first, bollinger_second = self._bb()
       

        if K_fast < 20 and D_slow < 20 and K_fast > D_slow and  bollinger_second > self.candles[-1].close:
            return 1
        elif K_fast > 80 and D_slow > 80 and K_fast < D_slow and bollinger_first < self.candles[-1].close:
            return -1
        else:
            return 0
